...
class JukeBoxApp:
    def __init__(self, window):
        ...
        # Update/Add Track Section
        self.update_frame = tk.Frame(self.notebook, bg="pink")
        self.notebook.add(self.update_frame, text="Update/Delete Tracks")
        self.track_id_entry = tk.Entry(self.update_frame, width=20)
        self.track_id_entry.pack(pady=5)
        self.track_name_entry = tk.Entry(self.update_frame, width=20)
        self.track_name_entry.pack(pady=5)
        self.track_artist_entry = tk.Entry(self.update_frame, width=20)
        self.track_artist_entry.pack(pady=5)
        self.track_rating_entry = tk.Entry(self.update_frame, width=20)
        self.track_rating_entry.pack(pady=5)
        self.track_youtube_entry = tk.Entry(self.update_frame, width=20)
        self.track_youtube_entry.pack(pady=5)

         # Update or Add Track Button
        self.update_button = tk.Button(self.update_frame, text="Update Track", command=self.update_or_add_track)
        self.update_button.pack(pady=10)

        # Delete Track Section
        self.delete_id_entry = tk.Entry(self.update_frame, width=20)
        self.delete_id_entry.pack(pady=5)
        self.delete_button = tk.Button(self.update_frame, text="Delete Track", command=self.delete_track)
    ...
    def update_or_add_track(self):
        ...
        artist = self.track_artist_entry.get().strip()
        rating = self.track_rating_entry.get().strip()
        youtube_link = self.track_youtube_entry.get().strip()

        if track_id and name and artist and rating and youtube_link:
            if track_id in library:
                ...
                messagebox.showinfo("Info", f"Track {track_id} updated successfully.")
            else:
                # Add new track
                library[track_id] = LibraryItem(track_id, name, artist, rating, youtube_link)
                messagebox.showinfo("Info", f"Track {track_id} added successfully.")
            save_library_to_csv("data_song.csv")
            self.load_tracks()  # Refresh the lists
        else:
            messagebox.showerror("Error", "Please fill all fields.")

    def delete_track(self):
        """Delete the track by ID."""
        track_id = self.delete_id_entry.get().strip()
        if track_id:
            if track_id in library:
                del library[track_id]
                save_library_to_csv("data_song.csv")
                self.load_tracks()  # Refresh the lists
                messagebox.showinfo("Info", f"Track {track_id} deleted successfully.")
            else:
